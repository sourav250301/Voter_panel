...
# import queries_EP
# import queries_BC
from COMBINE_EP_AND_PP_RAW_DATA_AND_CALCULATION import queries_BC
from COMBINE_EP_AND_PP_RAW_DATA_AND_CALCULATION import queries_EP
from COMBINE_EP_AND_PP_RAW_DATA_AND_CALCULATION import queries_PP
pd.set_option('display.max_columns', None)

# Function to fetch all query variables from query_Ep module
def fetch_all_queries_from_module(*modules):
    queries = []
...
def execute_queries_and_save_to_dataframe(server, database, username, password):
    ...
    try:
        with pyodbc.connect(connection_string, autocommit=True) as conn:
            cursor = conn.cursor()
            queries = fetch_all_queries_from_module(queries_PP,queries_EP,queries_BC)
